...
/* 4択で使う索引（2026-09-21）。ITEMS から外した選択肢の行（qid-1〜4・ア〜エ）も引く。 */
var RAWBY={};RAW.forEach(function(it){RAWBY[it.id]=it});
...
/* チェック用問題（本人が「これはいる・これはいらない」を判断する回）。
   2026-08-25 本人指摘「単語となぜのやつ、なんでチェック問題出してくれないの」。
   刻みは本人の指示どおり 1問→2→3→5→10（画面には小さい回が上に来る）。
   dataAt ＝ 用語・制度カードとその音を配ったデータ配信の時刻。端末がこれより古ければ出さない。 */
var CHECKS=[
  {label:"用語となぜ（1問）",unit:"用語となぜ（1問）",note:"答えたあとに、点線の語を押すと用語が出ます。電球の「なぜ・例」も押してください。要らない語・違う説明があれば「おかしいところ」から送ってください。",dataAt:"2026-08-25 17:03",ids:["b5_7-001-1"]},
  {label:"用語となぜ（2問目）",unit:"用語となぜ（2問目）",dataAt:"2026-08-25 17:03",ids:["b5_7-001-2"]},
  ...
  /* 合体文を短くする（2026-09-01 本人が文を示した）。本人の言葉：
     「この問題は／Aが媒介業者として関与する宅地の売買契約において、37条書面の電磁的方法に
     よる提供を行う場合であっても、提供後速やかに37条書面を交付しなければならない。／
     これで事足りると思うんだよね」。リードが略称の定義だけなので丸ごと落とした。 */
  {label:"合体文を短く（1問）",unit:"合体文を短く（1問）",
   note:"令和6年 問35（4）です。問題文とその読み上げを見てください。リード（「以下この問において「37条書面」という」等）を丸ごと落として、問題文だけにしました。ほかにも要らないリードがあれば「おかしいところ」から送ってください。",
   dataAt:"2026-09-01 00:30",ids:["b5_8-003-4"]}
];
...
function normST(o){
  ...
  /* 4択の記録（2026-09-21）。○×と同じく**過去問1問1答の成績には混ぜない**。
...
function checkList(){
  ...
  for(var i=0;i<a.length;i++){
    ...
    out.push({unit:c.unit||'',dataAt:c.dataAt||'',ids:ids,note:c.note||''});
  }
  return out;
}
/* ============ 模試（本試験形式・4択・時間を計る） ============
...
function checkHtml(){
  ...
  for(var i=0;i<list.length;i++){
    ...
    h+='<div class="dlv"><span class="dlv-b">配信しました</span><b>'+esc(c.unit||'チェック用問題')
      ...
      /* 何を見てほしいかを書いておく（2026-08-25）。note を持たせていたのに
         一度も画面に出しておらず、本人は何をチェックすればいいか分からなかった。 */
      +(c.note?('<div class="mini" style="margin:-2px 0 8px;padding:0 4px;line-height:1.7">'
        +esc(c.note)+'</div>'):'');
  }
  return h;
}
...
var OXB={
  ...
  /* 答えを1つ記録する。[答えた回数,正解した回数,最後に押した答え]
  ...
  /* やめた場所（2026-09-20 本人「途中から出来ないんだ」）。単元ごとに覚える。 */
  pos:function(d){
...
function vOx(){
  ...
  if(!done){
    ...
    h+='<div class="ans" style="margin-top:12px">'
      +'<button class="b" data-act="oxans" data-v="○">○</button>'
      +'<button class="b x" data-act="oxans" data-v="×">×</button>'
      +'</div>';
  }else{
    h+='<div class="oxjudge'+(good?' ok':' ng')+'">'+(good?'正解':'まちがい')
      ...
      +(sec?('<button class="btn sm" data-act="oxnote" data-d="'+esc(d)+'" data-s="'+sec+'">'
        +'ノートのこの節を読む</button>'):'')
      +'<button class="btn sm" data-act="oxagain">もう一度答える</button>'
      +'</div>';
  }
...
/* ---------- 過去問を4択のまま解く（2026-09-21 本人指示） ----------
   「単元学習で4択問題も出来るようにしておいて。過去問そのままのやつね」
   元＝data/questions.json →（tools/build_yon_js.py）→ data/yon.js（YONQ＝[正解番号,リード,型]）。
   選択肢の文と解説は**すでに端末にある RAW**（qid-1〜4＝選択肢そのもの／qid-ア〜エ＝
   個数・組合せの記述）から引く＝同じ文を二重に配らない。リードも肢も**一字も変えない**。
   記録は ST.yon に別で持つ＝○×と同じく、過去問1問1答の成績・到達度には混ぜない
   （1つ選ぶ4択と、肢ごとの○×は数え方が違う）。 */
var YB={
  ...
  opts:function(id){
    var o=[];
    for(var n=1;n<=4;n++){var r=RAWBY[id+'-'+n];o.push(r?r.stem:'')}
    return o;
  },
  /* 個数・組合せの「ア〜エ」。無ければ空（ふつうの4択は選択肢そのものが記述） */
  parts:function(id){
    var out=[],ks=['ア','イ','ウ','エ','オ'];
    for(var i=0;i<ks.length;i++){var r=RAWBY[id+'-'+ks[i]];if(r)out.push([ks[i],r])}
    return out;
  },
  /* 選択肢ごとの解説（ふつうの4択）。RAW の行が持っている */
  optRow:function(id,n){return RAWBY[id+'-'+n]||null},
  /* 単元ごとの問（出てくる順＝データの並び＝年度・問番号の順） */
  ofCat:function(c){
    if(!this._c){
      this._c={};
      if(this.ok())for(var i=0;i<RAW.length;i++){
        ...
        var a=this._c[it.cat]||(this._c[it.cat]=[]);
        if(a[a.length-1]!==it.qid)a.push(it.qid);
      }
    }
    return this._c[c]||[];
  },
...
function vUnit(c,s){
  ...
  var h='<div class="pad'+stag()+'">'
   ...
   +(NB.unit(c)?('<div style="display:flex;gap:8px;margin-top:8px">'
     ...
     +'論点ごとの到達度</button></div>'):'')
   /* 過去問を4択のまま解く（2026-09-21 本人指示）。1問1答（肢ごとの○×）とは別の入口。 */
...
document.addEventListener('click',function(e){
  ...
  if(a==='yonnext'||a==='yonprev'||a==='yongo'){
    ...
    if(a==='yongo'){
      ...
      var ypo=yq2.indexOf(S.yonI|0)+(a==='yonnext'?1:-1);
      if(ypo<0)ypo=0;
      yni=(ypo>=yq2.length)?ymax:yq2[ypo];
    }else{
      yni=(S.yonI|0)+(a==='yonnext'?1:-1);
    }
    if(yni<0)yni=0;
