/* 起動の演出（本人が採用した 11案）。tools/build_bootfx.py が作る。手で編集しない。
   ...
   ・**init はHTMLを差し込んだ後に呼ぶ**（先に呼ぶと要素が無く、地の色のまま止まる）
   ・prefers-reduced-motion のときは出さない（端末の設定を尊重する） */
(function(){
  /* 試験までの残り日数（2026-08-23 本人指摘「61日から変化しない」）。
     ここ1か所で出して、各案には data-days の目印で入れる＝案ごとに計算しない。
     日付の切り替わりは app.html と同じ「3時区切り」ではなく**暦の日**で数える
     （試験日までの日数なので、未明に見ても翌日扱いにしない）。 */
  var EXAM = '2026-10-18';
  function bootDays(){
    var p = EXAM.split('-');
    var ex = new Date(+p[0], +p[1]-1, +p[2]);
    var d = new Date(); d = new Date(d.getFullYear(), d.getMonth(), d.getDate());
    return Math.max(0, Math.round((ex - d) / 86400000));
  }
  window.TAKKEN_BOOTDAYS = bootDays();
  /* 差し込んだHTMLの中の目印に日数を入れる。*/
  function fillDays(box){
    var n = window.TAKKEN_BOOTDAYS, s = String(n), i, a;
    a = box.querySelectorAll('[data-days]');
    for(i=0;i<a.length;i++)a[i].textContent = s;
    a = box.querySelectorAll('[data-days-t]');
    for(i=0;i<a.length;i++)a[i].textContent = String(Math.floor(n/10)%10);
    a = box.querySelectorAll('[data-days-o]');
    for(i=0;i<a.length;i++)a[i].textContent = String(n%10);
  }
...
(function(){
  ...
  /* めくる先＝**試験までの残り日数**（2026-08-23 本人指摘。前は 61 に固定だった）。
     十の位は4枚前から、一の位は6枚前から回して、最後に本当の数字で止まる。 */
  var DAYS = (window.TAKKEN_BOOTDAYS || 61), TT = Math.floor(DAYS/10)%10, OO = DAYS%10;
  function upto(end, n){
    var a = [], i;
    for(i=n;i>=0;i--)a.push(String((end - i + 10) % 10));
    return a;
  }
  var SEQ_T = upto(TT, 4);
  var SEQ_O = upto(OO, 6);
...
(function(){
  ...
  window.TAKKEN_BOOTFX = {
    ...
    play: function(el, id){
      ...
      fillDays(wrap);                 /* 日数を入れてから差し込む（初期化より前） */
      el.appendChild(wrap);
      /* 差し込んだ**後**に初期化する＝この中で window.playBoot_X が定義される */
      if(typeof pick.init === 'function'){ try{ pick.init() }catch(e){} }
      var fn = window['playBoot_'+pick.id];
      if(typeof fn !== 'function'){
        /* 出せないなら覆いを残さない＝地の色で止めない */
